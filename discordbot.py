...
# 현재 디렉토리를 기준으로 절대 경로를 설정합니다.
current_dir = os.path.dirname(os.path.abspath(__file__))
inventory_file = os.path.join(current_dir, "data/inventory.json")
prices_file = os.path.join(current_dir, "data/prices.json")
...
@app_commands.autocomplete(item=lambda i, c: [app_commands.Choice(name=x, value=x) for x in creatures + items if c.lower() in x.lower()])
